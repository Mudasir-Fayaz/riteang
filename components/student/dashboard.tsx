...
interface StudentDashboardProps {
  user: AuthUser
  activeView: string // New prop to control active view
}

export function StudentDashboard({ user, activeView }: StudentDashboardProps) {
  ...
  const loadStudentData = async () => {
    try {
      ...
      // Load active jobs
      const { data: jobsData } = await supabase.from("jobs").select("*").eq("status", "active")
      setJobs(jobsData || [])
    } catch (error) {
  ...
  return (
    <div className="p-2 md:p-4 lg:p-6 space-y-3 md:space-y-6">
      <h2 className="text-3xl font-bold hidden md:block">Student Dashboard</h2> {/* Hide on mobile */}
      {activeView === "overview" && (
